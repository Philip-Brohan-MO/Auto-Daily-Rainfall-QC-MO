"""Precompute stage: write the SEF-animation run manifest (single SLURM job).

The SEF animation reuses the same deterministic frame plan as the ensemble
animation (``render_animation_manifest.py``) but records the SEF source root and
a millimetre colour scale, so the shared validate and encode stages can drive it
unchanged.
"""
...
from src.rainfall_rescue_sqlite.sef_animation import shard_bounds, total_frames
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the SEF-animation run manifest")
    parser.add_argument("--sef-root", type=Path, required=True)
    ...
    parser.add_argument("--error-color", default="#d62728")
...
def main() -> None:
    ...
    frame_dir = args.frame_dir or _pdir_path("sef_animation", "frames")
    output_path = args.output_path or _pdir_path(
        "sef_animation",
        f"sef_rainfall_{args.date_start.isoformat()}_{args.date_end.isoformat()}.mp4",
    )
    ...
    manifest = {
        "sef_root": str(Path(args.sef_root).resolve()),
