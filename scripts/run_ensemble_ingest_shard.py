...
Each shard parses its interleaved slice of the ensemble JSON files and writes a
complete, self-contained ensemble database for just that slice to
``<shard-dir>/ens_shard_<index>.sqlite``. The shards are combined afterwards by
``merge_ensemble_shards.py``.

The JSON source tree is read-only, so many shards run concurrently. Each shard
writes its SQLite file on node-local scratch and then publishes it to the shared
shard directory (the shared parallel filesystem can't do SQLite write-locking).
"""
